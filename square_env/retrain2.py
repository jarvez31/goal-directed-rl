...
for ii,jj in zip(go_in_ind[:-1], go_out_ind[:-1]):
    for i in range(ii+5500,jj+1):
        R[i] = 0

for j in range(go_in_ind[-1]+5500, len(R)):
    R[j] = 0

plt.scatter(go_in_ind, [0]*len(go_in_ind), label = "enters")
plt.scatter(go_out_ind, [0]*len(go_out_ind), label = "leaves")
plt.plot(range(len(R)), R, label = "Reward")
plt.legend()
plt.title("Temporal Reward function")
plt.show()
...
if Train:
    ...
    plt.plot(range(len(V)), V, label = "value")
    g = pos[f[0], :]
    ...
    for l in go_out_ind:
        plt.plot([l]*len(p), p)
    plt.plot(go_in_ind, [0]*len(go_in_ind), 'bo', label = "enters")
    plt.plot(go_out_ind, [0]*len(go_out_ind), 'ro', label = "leaves")
    plt.plot(range(len(R)), R, color='Black', label = "reward")
    plt.legend()
    plt.title("value function for plus maze")# of " + str(retrain_eps) + ' epochs')
    plt.show()
