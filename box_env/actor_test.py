...
import numpy as np
from GEN_func import model_trained, HD, PI, all_plot, hd, speedpos, speedinit, rew, new_circle, cir_check, mirrorImage, sq_chk, updatestep
from shapely.geometry import LineString, LinearRing, box, Point, Polygon
import matplotlib.pyplot as plt
from actor import neural_network2
# from GEN import neural_network
...
def neural_network():
    ...
    hid_lay1 = {'weights': tf.Variable(tf.random_normal([49, nodes[0]]),dtype=tf.float32, name='wts_hid_lay1'),
    ...
    output_lay = {'weights': tf.Variable(tf.random_normal([nodes[-1], 1]), dtype=tf.float32, name='wts_output_lay'),
...
def step_cal(tf_session, path, L2, heading, model):
    # with tf.Session() as sess:
    saver = model["saver"]
    saver.restore(tf_session, path)        
    value = []
    x_t = np.append(L2, heading)
    output_update = tf_session.run(model["output"], feed_dict = {model["x"]:x_t.reshape((1,149))})
...
bowl_rad = 0.5
bowl_cen = [(1.5,0), (0, 1.5), (-1.5, 0), (0, -1.5)]
bowl_ind = [0,1,2,3]
p = [Point(i) for i in bowl_cen]
circles = [pp.buffer(bowl_rad) for pp in p]
...
save_path2 = fol1 + '/actor{lay:' + str(len(nodes_act)) + " neu:"
for k in nodes:
    save_path = save_path + str(k) +','
save_path2 = save_path2 + str(nodes_act[0]) +','
...
for T in range(200000):
    ...
    with tf.Session() as sess2:
        newdelX, newdelY = step_cal(sess2, save_path2, ot2_part, head, nw2)
    ...
    if T%1000==0:
        ...
        with open('traj_GEN_actf2.pk1', 'wb') as f:
            pickle.dump(d, f)
